...
bool compareCost(const class Job *a , const class Job *b) {
    return (a->priority * ((a->ntask-1) / nWorker + 1) * a->taskTime + b->priority * (((a->ntask-1) / nWorker + 1) * a->taskTime + ((b->ntask-1) / nWorker + 1) * b->taskTime)
            < b->priority * ((b->ntask-1) / nWorker + 1) * b->taskTime + a->priority * (((a->ntask-1) / nWorker + 1) * a->taskTime + ((b->ntask-1) / nWorker + 1) * b->taskTime) ); // a->cost < b->cost
}
